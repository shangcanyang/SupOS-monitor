...
- **画布节点**：30 种节点（数据源 / 判断 / 逻辑 / 流程控制 / 时间 / 状态记忆 / 运算 / 变量 / 触发），参考米家自动化极客版风格
  - 数据源：位号取值 / 布尔位号 / 位号质量 / 常量
  - 判断：比较 / 区间 / 偏差 / 变化率 / 文本判断（含正则）
  - 逻辑：逻辑与或 / 非 / 异或
  - 流程：条件分支（if/else） / 合并分支
  - 时间：持续时间 / 延时 / 保持（防抖） / 脉冲 / 时间段
  - 状态记忆：锁存（SR/RS） / 翻转 / 边沿检测 / 计数器
  - 运算：算术运算 / 数学函数 / 线性映射 / 窗口统计
  - 变量：读变量 / 写变量
  - 触发：触发报警
  - 报警保持：自动复位 / 定时保持 / 保持到人工确认
...
| 判断 | 比较、区间、偏差、变化率、文本判断（含正则） |
| 逻辑 | 逻辑与/或、非、异或 |
| 流程控制 | 条件分支（if/else）、合并分支 |
| 时间 | 持续时间、延时、保持（防抖）、脉冲、时间段 |
| 状态 / 记忆 | 锁存（SR/RS）、翻转、边沿检测、计数器 |
| 运算 | 算术运算、数学函数、线性映射、窗口统计 |
| 变量 | 读取变量、写入变量 |
| 触发 | 触发报警 |

**状态记忆**：锁存、翻转、边沿检测、计数器、保持、脉冲、窗口统计等节点带跨 tick 状态，变量值与节点状态会写入 `%APPDATA%` 下的 `canvas-state.json`，程序重启后继续沿用，不会从零开始。

**报警保持**：触发报警节点可选三种保持方式——

- **自动复位**：条件消失即解除报警
- **定时保持**：条件消失后再保留 N 秒（可配）
- **保持到人工确认**：锁存报警，需在报警窗口点「确认复位」才解除。确认时会一并清空该规则内所有状态节点（锁存 / 计数器等）的记忆，条件若已恢复则立即解除；条件仍成立则保持显示且不再重复推送，待条件恢复后再次成立时按新一次报警处理
...
画布规则（RulesEngine 每秒 tick）：
  1. 从 trigger 节点反向追踪
  2. 递归求值每个节点（带状态节点按 tick 幂等更新并记忆）
  3. 命中 → onAlarm（按保持模式决定何时复位）
  4. 变量值与节点状态有变化 → 节流写入 canvas-state.json
```
...
7. 需要跨 tick 记忆状态的节点，用 `nst()` / `tickState()` 读写；状态字段为数字 / 布尔 / 字符串时会由 `canvas-state.js` 自动落盘（`hist` 等大数组不落盘）
...
- 画布变量值与节点状态落盘于用户数据目录（`%APPDATA%\SupOS-monitor\canvas-state.json`），重启后延续；需彻底清零时删除该文件即可
- 「保持到人工确认」的报警需在报警窗口点「确认复位」后才解除
- 邮件指令仅支持 QQ 邮箱 IMAP
